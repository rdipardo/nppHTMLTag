...
h3. Current Versions (1.5.0, ...)

* https://github.com/rdipardo/nppHTMLTag/releases
...
* Unicode character encoding/decoding (example: @é@ to @\u00E9@)
* Decoding (GitHub) emoji "shortcodes":sc (example: @:clown_face:@ to 🤡)
* autocompletion of HTML entities and emoji shortcodes
...
| Ctrl+Shift+E | to _decode_ all selected HTML entities or emoji shortcodes |
| Alt+J | to encode all selected non-ASCII characters to their Unicode code point values |
| Alt+Shift+J | to _decode_ all selected Unicode escape sequences |
...
* -Add command to select tags only (and leave the contents out of the selection)-
  Added in "1.4.1":https://github.com/rdipardo/nppHTMLTag/blob/v1.4.1/NEWS.textile
* Make it play nice(r) with PHP & ASP tags (either highlight those separately or ignore them)
* -Add option to encode entities 'as-you-type'- Added in "1.4.0":https://github.com/rdipardo/nppHTMLTag/blob/v1.4.0/NEWS.textile
...
[sc]https://emojipedia.org/shortcodes
